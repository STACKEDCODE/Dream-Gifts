...
import { createBuy, getBuys, deleteBuy, exportExcel } from "../lib/buys";
...
export default function Compras() {
    ...
    const [buys, setBuys] = createSignal([])
    ...
    const fetchBuys = async () => {
        setLoading(() => true)
        const buys = await getBuys(token).then((response) => { return response.data })
    ...
    function ModalAñadir({ name }) {
        ...
        const handleSubmit = (event: Event): void => {
            ...
            const { folio, fecha, idProveedor, valorNeto } = event.target as HTMLFormElement
            let buy: Buy = {
                folio: folio.value,
                fecha: fecha.value,
            ...
            createBuy(buy, token).then(() => fetchBuys()).catch((error) => {
                console.log(error)
            })
            closeModal()
        }
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6 p-4">
                            ...
                            <div class="flex flex-col mb-4">
                                ...
                                <select name="idProveedor" id="idProveedor" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                                    ...
                                    <For each={providers()}>
                                        {(provider) => (
                                            <option value={provider.id}>{provider.razonSocial}</option>
                                        )}
                                    </For>
                                </select>
                            </div>
    ...
    const deleteItem = async (event: Event): Promise<void> => {
        ...
        await deleteBuy(id.value, token).then(
            () => fetchBuys()
        ).catch((error) => {
            console.log(error)
        })

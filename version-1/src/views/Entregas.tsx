...
import { createDelivery, getDeliveries, deleteDelivery, exportExcel } from "../lib/deliveries";
import { createEffect, createSignal } from "solid-js";
import { Show, For } from "solid-js";
import Table from "../components/Tabla"
import styles from "../styles"
export default function Entregas() {
    let name = "modalAñadir"

    const [deliveries, setDeliveries] = createSignal([])
    const [loading, setLoading] = createSignal(true)
    const token = sessionStorage.getItem("token")

    const fetchDeliveries = async () => {
        setLoading(true)
        const deliveries = await getDeliveries(token).then((response) => { return response.data }).catch((error) => { console.log(error) })
    ...
    function ModalAñadir({ name }) {
        ...
        const handleSubmit = async (event: Event): Promise<void> => {
            ...
            const { idVenta, fecha, direccion, valor, estado } = event.target as HTMLFormElement
            ...
            await createDelivery(delivery, token).then(
                () => fetchDeliveries()
            ).catch((error) => {
                console.log(error)
            })
                    ...
                    <form onSubmit={handleSubmit} class="relative bg-white rounded-lg shadow">
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6 p-4">
                            <div class="flex flex-col mb-4">
                                <label for="idVenta" class="block mb-2 text-sm font-medium text-gray-900">N° Venta</label>
                                <input type="text" name="idVenta" id="idVenta" placeholder="idVenta" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" />
                            </div>
                            <div class="flex flex-col mb-4">
                                <label for="fecha" class="block mb-2 text-sm font-medium text-gray-900">Fecha</label>
                                <input type="date" name="fecha" id="fecha" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" />
                            </div>
                            <div class="flex flex-col mb-4">
                                <label for="direccion" class="block mb-2 text-sm font-medium text-gray-900">Dirección</label>
                                <input type="text" name="direccion" id="direccion" placeholder="Dirección" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" />
                            </div>
                            <div class="flex flex-col mb-4">
                                <label for="valor" class="block mb-2 text-sm font-medium text-gray-900">Valor</label>
                                <input type="number" name="valor" id="valor" placeholder="Valor" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" />
                            </div>
                            <div class="flex flex-col mb-4">
                                <label for="estado" class="block mb-2 text-sm font-medium text-gray-900">Estado</label>
                                <select name="estado" id="estado" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
    ...
    const deleteItem = async (event: Event): Promise<void> => {
        ...
        await deleteDelivery(id.value, token).then(
            () => fetchDeliveries()
        ).catch((error) => {
            console.log(error)
        })
        ...
        <main>
            ...
            <Content>
                ...
                <div class="grid grid-cols-3 gap-2">
                    <button type="button" class={styles.buttonDefault} onClick={openModal}>Agregar entrega</button>

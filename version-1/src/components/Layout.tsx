...
import { CreditCardIcon, HomeIcon, UserIcon, GiftIcon, ChatBubbleLeftRightIcon, CalendarDaysIcon } from "./Icons";
...
export function Navbar() {
    ...
    return (
        <div class="bg-[#38D484] dark:bg-gray-800 fixed top-0 w-full z-50">
        ...
        <div>
            ...
            >
                <aside id="default-sidebar" class="fixed pt-16 z-0 top-0 left-0 z-40 border-r w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
                    <div class="h-full px-3 py-8 xl:py-4 overflow-y-auto bg-white dark:bg-gray-800">
                            ...
                            >
                                {
                                    (item) => (
                                        <Show when={item.href === location.pathname}
                                            fallback={
                                                <A href={item.href} class="flex items-center p-2 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                                                    <item.icon style={styles.sideIcon} />
                                                    <span class="ml-2 text-2xl font-bold dark:text-white text-gray-600">{item.name}</span>
                                                </A>
                                            }
                                        >
                                            <A href={item.href} class="flex items-center p-2 rounded-lg bg-[#38D484] dark:text-white hover:bg-green-500 dark:hover:bg-gray-700 group">
...
export function Footer() {
    return (
        <footer class="bg-[#737373] dark:bg-gray-800 fixed bottom-0 w-full z-40">
